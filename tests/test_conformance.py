...
def pytest_generate_tests(metafunc):
    ...
    for level in ("level_{0:d}".format(level) for level in [1, 2, 3]):
        ...
        test_path = root_path.join(
            "..", "conformance_tests", "upstream", level
        )

        tests = list(get_tests(test_path))
        argvalues.extend([test[:3] for test in tests])
        ids.extend(["{0:s}_{1:s}".format(level, test[-1]) for test in tests])
...
def test(selector, document, output):
